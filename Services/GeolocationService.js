...
class GeolocationService {
    ...
    getAutocompleteSuggestions = async (currentLocationInfo, prompt) => {
        ...
        const { data: reverseGeocodePayloadData } = await this.getReverseGeocoding(longitude, latitude);
        const currentCountry = reverseGeocodePayloadData?.[0]?.country_code || 'in';
